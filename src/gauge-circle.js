  ...
  properties: [
    ...
    },
    {
      ...
      observe: function(startValue) {
        this.parentNode
          .querySelector("[name=colorStops]")
          .set("property.min", startValue);
      },
      property: "startValue"
    },
    {
...
export default class GaugeCircle extends ValueHolder(Donut) {
  _draw(context) {
    ...
    context.ellipse(
      ...
      Math.abs(ry),
      0,
      (startAngle * Math.PI).toFixed(2),
      (endAngle * Math.PI).toFixed(2)
    );
    ...
    if (colorStops) {
      ...
      colorStops.forEach(function(v, idx, arr) {
        ...
        let startStepAngle =
          endStepAngle ||
          Math.PI * (startAngle + (circleSize * beforeValue) / totalValue);
        ...
        if (idx === arr.length - 1)
          ...
          endStepAngle =
            Math.PI * (startAngle + (circleSize * value) / totalValue);
    ...
    let ang =
      Math.PI *
      ((circleSize * (drawingValue - startValue)) / totalValue +
        startAngle -
        0.5);
    ...
    if (showStepLine) {
      ...
      drawStepLine(
        context,
        Math.PI * (startAngle + 0.5),
        rx * 0.8,
        stepNeedleSize
      ); // 원을 그릴때 PI는 45도 부터 그리지만 angle은 0도부터 틀어서 + 0.5도를 곱해줘야함
      // Draw StepValue
